...
def load_data():
    if os.path.exists("data.xlsx"):
        ...
        # 'no' 또는 '번호' 컬럼 찾기 (대소문자 무시)
        col_list = df.columns.tolist()
        target_col = next((c for c in col_list if c.lower() == 'no' or c == '번호'), None)
        
        if target_col:
            # 해당 컬럼 기준으로 데이터 정제
            df = df.dropna(subset=[target_col])
            df[target_col] = pd.to_numeric(df[target_col], errors='coerce').fillna(0).astype(int)
            df = df[df[target_col] > 0].sort_values(by=target_col)
            # 이후 사용을 위해 컬럼명을 'no_val'로 통일 (내부 참조용)
            df['no_val'] = df[target_col]
        return df
    return pd.DataFrame()
...
if not df.empty:
    ...
    for _, row in df_filtered.iterrows():
        # [핵심 수정] 엑셀의 'no' 혹은 '번호' 값을 직접 가져옴 (순번 사용 안 함)
        actual_no = row.get('no_val')
    ...
    # 내부용 컬럼 삭제
    if 'no_val' in display_df.columns:
        display_df = display_df.drop(columns=['no_val'])
