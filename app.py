...
location_map = {
    ...
    "순성면": [36.8531, 126.6854]  # <-- 당진시 순성면 추가
}
    ...
    if os.path.exists("data.xlsx"):
        try:
            ...
            if '번호' in df.columns:
                ...
                df = df[df['번호'] > 0].sort_values(by='번호', ascending=False) # 최신순 정렬
            return df
...
if not df.empty:
    ...
    search = st.sidebar.text_input("지역 또는 내용 검색 (예: 순성면)")
    ...
    st.warning("데이터가 없습니다. 엑셀 파일(data.xlsx)이 있고, 컬럼명이 '번호', '시군'으로 시작하는지 확인해 주세요.")
