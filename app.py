...
def load_data():
    if os.path.exists("data.xlsx"):
        ...
        # [핵심] 첫 번째 컬럼을 무조건 '번호_기준'으로 사용 (컬럼명이 no든 번호든 상관없음)
        first_col = df.columns[0]
        df = df.rename(columns={first_col: '번호_기준'})
        
        # 데이터 정제
        df = df.dropna(subset=['번호_기준'])
        df['번호_기준'] = pd.to_numeric(df['번호_기준'], errors='coerce').fillna(0).astype(int)
        df = df[df['번호_기준'] > 0].sort_values(by='번호_기준')
...
if not df.empty:
    ...
    for _, row in df_filtered.iterrows():
        # [수정] 엑셀에 적힌 실제 번호만 가져옴 (컴퓨터 순번 사용 안함)
        display_num = int(row['번호_기준'])
    ...
    # 목록에서 보기 좋게 다시 컬럼명 변경
    display_df = display_df.rename(columns={'번호_기준': '번호'})
    ...
    st.warning("데이터가 없습니다. 엑셀 파일의 첫 번째 열이 발생 번호인지 확인해 주세요.")
