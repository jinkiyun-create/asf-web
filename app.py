...
if not df.empty:
    ...
    for _, row in df_filtered.iterrows():
        ...
        if coords:
            # 번호와 정보 추출
            item_no = row.get('번호', '0')
            city_name = row.get('시군', '지역명 없음')
            scale = row.get('사육규모', 0)
            content = row.get('발생내용', '내용 없음')

            # 팝업 HTML (번호 확실하게 표시)
            popup_html = f"""
                <div style="width:180px; font-family: 'Malgun Gothic';">
                    <b style="font-size:14px; color:#d32f2f;">[{item_no}번] {city_name}</b><br>
